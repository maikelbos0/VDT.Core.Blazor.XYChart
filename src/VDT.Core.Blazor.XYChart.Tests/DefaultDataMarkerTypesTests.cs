...
public class DefaultDataMarkerTypesTests {
    [Fact]
    public void Round() {
        var result = DefaultDataMarkerTypes.Round(50M, 150M, 10M, "red", "example-data", 1, 2, 5);
        ...
        Assert.Equal($"{nameof(RoundDataMarkerShape)}[1,2,5]", shape.Key);
        Assert.Equal("data-marker data-marker-round example-data", shape.CssClass);
    }

    [Fact]
    public void Square() {
        var result = DefaultDataMarkerTypes.Square(50M, 150M, 10M, "red", "example-data", 1, 2, 5);
        ...
        Assert.Equal(10M, shape.Size);
        Assert.Equal("red", shape.Color);
        Assert.Equal($"{nameof(SquareDataMarkerShape)}[1,2,5]", shape.Key);
