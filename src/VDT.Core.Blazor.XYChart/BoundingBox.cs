...
public record BoundingBox(decimal X, decimal Y, decimal Width, decimal Height) {
    /// <summary>
    /// Required width to make the element fit inside an area, taking any offset of the X-coordinate into account
    /// </summary>
    public int RequiredWidth => (int)Math.Ceiling(Math.Abs(X) > Width ? Math.Abs(X) : Width + Math.Abs(X));

    /// <summary>
    /// Required height to make the element fit inside an area, taking any offset of the Y-coordinate into account
    /// </summary>
